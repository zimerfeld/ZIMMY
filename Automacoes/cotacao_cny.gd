...
const AUTOMATION_NAME_ES := "Cotización CNY"
...
func run(zimmy) -> void:
	zimmy.notify(zimmy.lang_text("buscando CNY... 🌐", "fetching CNY... 🌐", "buscando CNY... 🌐"))
	zimmy.http_get_json("https://economia.awesomeapi.com.br/last/CNY-BRL", func(ok, data):
		if ok and data is Dictionary and data.has("CNYBRL"):
			...
			zimmy.notify(zimmy.lang_text("falha na cotação CNY 🌐", "CNY quote failed 🌐", "fallo en la cotización CNY 🌐"))
	)
