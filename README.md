...
- ![EN](screenshots/screenshotGB.png) **🦴 Feed / 🤚 Pet / 🎾 Play** — interactions that change mood/hunger. Each one also plays its **own sound** (a munch, a purr, a playful arpeggio) when the alert for that action is on.
...
- ![PT](screenshots/screenshotBR.png) **🦴 Alimentar / 🤚 Carinho / 🎾 Brincar** — interações que mudam humor/fome. Cada uma também toca um **som próprio** (uma mordidinha, um ronron, um arpejo alegre) quando o alerta daquela ação está ligado.
