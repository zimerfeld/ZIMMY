...
- **Repo:** https://github.com/zimerfeld/ZIMMY · **branch:** `develop` (`75aed14`) · **`main`:** `53a390e` (in sync with origin, after the controls-collision hotfix).
...
## ✅ Recently done
- [x] **New release with today's tag + clean title** — 2026-07-07: created release **`202607072157`** (`gh release create … --title "202607072157"`), now **Latest** — title = tag only (dropped the descriptive name "Zimmy Pet — mouse reactions…"), bilingual 🇺🇸/🇧🇷 notes reused, `ZimmyPet.exe` (101 MB) attached. Tag created on `main` (`53a390e`) and pushed. Previous releases were **kept**. Same treatment applied to ZIMARO (`202607072141`). Procedure (incl. "create a new release with a new tag") documented in [[📦 Atualizar Asset da Release (GitHub) (EN)|Update the Release Asset (GitHub)]] (PT/EN).
- [x] **Landing-page fix — controls ("Quick reference") collision in PT** — 2026-07-07: same i18n template as ZIMARO. The rule `html[data-lang="pt"] .block.lang-pt{display:block}` forced the pills container to `display:block` in PT, making the `.pill`s **inline** — and inline vertical padding overlapped the backgrounds between lines (EN was fine, since there the container is `display:flex`). **1-line CSS fix:** `html[data-lang="pt"] .pills.lang-pt.block{display:flex}` restores flex/wrap in PT only. Shipped as a hotfix: fix on `develop` (`75aed14`) and cherry-picked to `main` (`53a390e`); GitHub Pages redeployed live at **zimmy.zimerfeld.com**. `.obsidian/*` kept out of the commit, as usual. **Release procedure documented in the vault:** [[📦 Atualizar Asset da Release (GitHub) (EN)|Update the Release Asset (GitHub)]] (PT/EN) under 🚀 Operação — how to swap/publish `ZimmyPet.exe` with `gh`, bypassing git.
