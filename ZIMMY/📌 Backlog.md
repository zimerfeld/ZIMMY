...
- **Repo:** https://github.com/zimerfeld/ZIMMY · **branch:** `develop` (`75aed14`) · **`main`:** `53a390e` (em sincronia com o origin, após o hotfix de colisão dos controles).
- **Últimas releases:** **`202607072157`** é a **Latest** (título = só a tag, notas bilíngues, `ZimmyPet.exe` anexado, criada em `main` `53a390e`). Anteriores mantidas: `202607071044geracao-30s` (geração 30s), `202607071024reacoes-e-fala` (reações/fala) e `202606251217`.
...
## ✅ Feito recente
- [x] **Nova release com tag de hoje + título limpo** — 2026-07-07: criada a release **`202607072157`** (`gh release create … --title "202607072157"`), agora **Latest** — título = só a tag (removido o nome descritivo "Zimmy Pet — mouse reactions…"), notas bilíngues 🇺🇸/🇧🇷 reaproveitadas, `ZimmyPet.exe` (101 MB) anexado. Tag criada em `main` (`53a390e`) e enviada ao remoto. As releases anteriores foram **mantidas**. Mesmo tratamento aplicado ao ZIMARO (`202607072141`). Procedimento (incl. "criar nova release com nova tag") documentado em [[📦 Atualizar Asset da Release (GitHub)]] (PT/EN).
- [x] **Fix na landing page — colisão dos controles ("Referência rápida") em PT** — 2026-07-07: mesmo template i18n do ZIMARO. A regra `html[data-lang="pt"] .block.lang-pt{display:block}` forçava o container dos pills para `display:block` no PT, tornando os `.pill` **inline** — e o padding vertical de inline sobrepunha os fundos entre linhas (em EN funcionava, pois lá o container fica `display:flex`). **Correção de 1 linha de CSS:** `html[data-lang="pt"] .pills.lang-pt.block{display:flex}` restaura flex/wrap só no PT. Publicado como hotfix: fix no `develop` (`75aed14`) e cherry-pick p/ `main` (`53a390e`); GitHub Pages redeployado ao vivo em **zimmy.zimerfeld.com**. `.obsidian/*` ficou fora do commit, como de praxe. **Procedimento de release documentado no cofre:** [[📦 Atualizar Asset da Release (GitHub)]] (PT/EN) na pasta 🚀 Operação — como trocar/publicar o `ZimmyPet.exe` com `gh` sem passar pelo git.
